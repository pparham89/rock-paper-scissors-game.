...
import turtle
import sqlite3
con = sqlite3.connect('c:/Users/89parham/Documents/rps/database rps.db')
cur = con.cursor()
...
def game():
    ...
    def setting():
        ...
        if state==True:
            ...
            sscale.place(x=700,y=150)
            state=False
        else:
            sscale.place_forget()
            state=True
        def change():
            lbl_laps.config(font=("bold ",ffont.get()))
            lbl_blaps.config(font=("bold ",ffont.get()))
            lbl_bot.config(font=("bold ",ffont.get()*2))
            lbl_shape.config(font=("bold ",ffont.get()*2))
            lbl_nname.config(font=("bold ",ffont.get()))
            lbl_resalt.config(font=("bold ",ffont.get()))
            btn_paper.config(font=("bold ",ffont.get()*2))
            btn_rock.config(font=("bold ",ffont.get()*2))
            btn_si.config(font=("bold",(ffont.get())*(2)))
            lbl_name.config(font=("bold",ffont.get()))

        sscale.config(command=lambda event: change())

    sscale = Scale( from_=10, to=38, variable=ffont, label='Size', orient=HORIZONTAL,background='purple',font='bold 15')
    btn_setting=Button(text='size setting',font='bold 30',bg='purple',command=setting)
    btn_setting.place(x=650,y=50)
    #player
    lbl_nname=Label(text=ffname,font='bold 30',bg='#278b10')
    lbl_nname.place(x=300,y=50)
